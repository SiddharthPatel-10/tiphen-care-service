...
const addSubscriptionToUser = async (req, res) => {
  const { userId, subscriptionPlan } = req.body; // Assuming subscriptionPlan is an object with plan details
  ...
  try {
    ...
    user.subscriptions.push(subscriptionPlan); // Push the subscription plan to user.subscriptions array
    await user.save();

    res
      .status(201)
      .json({ message: "Subscription added to user profile", user });
  } catch (error) {
